...
def fetch_vulnerabilities(ecosystem, package_name, package_version):
    ...
    while True:
        ...
        for edge in data["data"]["securityVulnerabilities"].get("edges", []):
            node = edge["node"]
            if is_version_vulnerable(package_version, node["vulnerableVersionRange"]):
...
# Additional test cases for suffix handling
fetch_npm_vulnerabilities("org.springframework:spring-core", "2.5.6.SEC02")  # Should not be vulnerable if range is <2.1.1
# fetch_npm_vulnerabilities("example-package", "2.0.0Alpha")  # Should be vulnerable if range is <2.1.1Alpha